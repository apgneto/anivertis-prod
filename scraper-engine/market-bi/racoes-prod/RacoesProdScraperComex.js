...
function agruparPorMes(data) {
  ...
  const mapa = {};
  for (const r of rows) {
    const ym = String(r.coAnoMes || r.month || r.period || '').replace('-', '');
    if (!ym) continue;
...
async function executarFlow(flow, ativoId) {
  ...
  const body = { flow, monthDetail: true, period: ultimos24Meses(), filters: [{ filter: 'ncm', values: ['23099010', '23099090'] }], details: ['ncm'], metrics: ['metricFOB'] };
  ...
  for (const mes of meses) {
    ...
    novosRegistros += await salvarPreco(ativo, dataRef, mes.valor.toString(), mes.valor, JSON.stringify(response.data).slice(0, 200000));
  }
  return novosRegistros;
}
async function scrapeRacoesProdComex() {
  try {
    const exp = await executarFlow('EXP', 'EXPORT_RACAO_PROD_COMEX');
    const imp = await executarFlow('IMP', 'IMPORT_RACAO_PROD_COMEX');
