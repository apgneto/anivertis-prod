...
const ATIVO = { ativo_id: 'EXPORT_SUINO_SECEX', unidade_origem: 'USD', unidade_destino: 'USD', fonte: 'MDIC_COMEXSTAT', tier: 1 };
...
function parseRows(data) {
  ...
  return rows.map((r) => ({ data: r.coAnoMes || r.month || r.period, valor: Number(r.metricFOB || r.vl_fob || r.value || 0) })).filter((r) => Number.isFinite(r.valor));
}
async function scrapeSuinosSecex() {
  const period = ultimos24Meses();
  const body = { flow: 'EXP', monthDetail: true, period, filters: [{ filter: 'ncm', values: ['0203'] }], details: ['ncm'], metrics: ['metricFOB'] };
  try {
    ...
    for (const row of rows) {
      ...
      novosRegistros += await salvarPreco(dataReferencia, row.valor.toString(), row.valor, JSON.stringify(response.data).slice(0, 200000));
    }
